...
const AdoptionRequestsModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-all duration-300">
      <div className="w-full max-w-2xl bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-2xl shadow-black relative animate-in fade-in zoom-in-95 duration-200 transition-colors duration-300">
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-[#111827] transition-colors duration-300">
          <div>
        ...
        <div className="overflow-x-auto max-h-[380px]">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800/60 text-slate-500 dark:text-slate-400 text-[11px] font-bold uppercase tracking-wider bg-slate-50 dark:bg-[#0B0F19]/50 transition-colors duration-300">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40">
              {requests.map(req => (
                <tr
                  key={req._id}
                  className="hover:bg-slate-50/80 dark:hover:bg-[#1E293B]/10 transition duration-150"
                >
                  <td className="py-3 px-5 whitespace-nowrap">
                    <div className="flex items-center gap-2.5">
                      <Image
                        ...
                        width={40}
                        height={40}
                        className="w-8 h-8 rounded-full object-cover border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900"
                      />
                      <div>
                  ...
                  <td className="py-3 px-4 text-xs text-slate-600 dark:text-slate-400 font-medium whitespace-nowrap">
                  ...
                  <td
                    className="py-3 px-4 text-xs text-slate-600 dark:text-slate-400 max-w-[160px] truncate"
                    title={req.message}
                  >
                    {req.message}
                  </td>
                        ...
                        className={`px-3 py-1 rounded-lg text-[10px] font-bold border transition
                            ${
                              req.status === 'pending'
                                ...
                                : 'bg-slate-100 dark:bg-[#1E293B]/20 border-slate-200 dark:border-slate-800/80 text-slate-400 dark:text-slate-600 cursor-not-allowed'
                            }`}
                      >
                        Approve
                      </button>
